...
##### Or to slightly reduce the time complexity, by sorting the guests info array based firstly on the check-in date and then the check-out date, so that when iterate the query dates
